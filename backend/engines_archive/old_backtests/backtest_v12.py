...
V12_SEASON = {
    'summer':  {'b':68,'h':60,'t1':12,'t2':9,'p4':60,'p4e':15,'tr':18,'t2on':True},
    'autumn':  {'b':75,'h':25,'t1':8, 't2':6,'p4':65,'p4e':5,'tr':12,'t2on':True},
    'spring':  {'b':70,'h':20,'t1':8, 't2':6,'p4':60,'p4e':5,'tr':12,'t2on':True},
    'chaos_spring':{'b':99,'h':5,'t1':3,'t2':3,'p4':999,'p4e':0,'tr':8,'t2on':False,'maxpos':0},
    'chaos':   {'b':99,'h':5,'t1':3,'t2':3,'p4':999,'p4e':0,'tr':8,'t2on':False,'maxpos':0},
    'chaos_autumn':{'b':99,'h':5,'t1':3,'t2':3,'p4':999,'p4e':0,'tr':8,'t2on':False,'maxpos':0},
...
def backtest():
    ...
    cur.execute("SELECT DISTINCT trade_date FROM daily_kline_qfq WHERE trade_date BETWEEN '2023-07-05' AND '2026-06-12' ORDER BY trade_date")
    ...
    for i, td in enumerate(dates):
        ...
        sea = gs(td)
        p = V12_SEASON.get(sea, V12_SEASON['chaos'])
