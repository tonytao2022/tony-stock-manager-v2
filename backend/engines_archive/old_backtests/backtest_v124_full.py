#!/usr/bin/env python3
"""
V12.4 全量回测 — 真实引擎(score_stock含强势股回退A轨) + V12.4季节参数矩阵(MAY优化版)
参数基于V12.2回测数据分析：summer买入线68→65、仓位40%→50%；chaos系买入线75→72

回测方式：每天对候选池逐只评分→买入，无简化
评分引擎：p6_dual_track_engine.score_stock (V12.4改版，含_is_strong_stock)
"""
...
# V12.4 季节参数矩阵（MAY优化版 v2.0）
# MAY建议改动：summer买入线68→65、仓位40%→50%；chaos系买入线75→72；
# spring(仅1笔样本)归入chaos_spring；chaos_autumn归入autumn(合并简化)
V124_PARAMS = {
    'summer':  {'buy':65, 'hold':60, 't1':12, 't2':9,  'p4_min':55, 'p4_ext':15, 'trailing':18, 't2_on':True,  'max_pos_pct':40},  # 仅改此行: 买入线68→65
...
def backtest():
    ...
    for i, td in enumerate(dates_rs):
        ...
        sea = get_season(td)
        cfg = V124_PARAMS.get(sea, V124_PARAMS['chaos'])
    ...
    print("\n" + "="*65, flush=True)
    print("📊 V12.4 全量回测（MAY优化版 — Summer加仓/Chaos降门槛/合并简化）", flush=True)
    print("="*65, flush=True)
    ...
    print("\n", flush=True)
    print("--- V12.2 vs V12.4 对比 ---", flush=True)
    print("V12.2: +167.31% | 262笔 | 45.04% | PF2.17 | 均持49.3d", flush=True)
    print("V12.4: %+.2f%% | %d笔 | %.2f%% | PF%.2f | 均持%.1fd" % 
          (total_ret, len(trades), wr, pf, ah), flush=True)
    
    res = {'version':'V12.4','initial_capital':INIT_CAPITAL,'final_value':round(fv,2),
    ...
    out = '/tmp/backtest_v124_result.json'
