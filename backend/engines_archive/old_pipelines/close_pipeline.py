...
拉取: daily_basic(5500+全市场) + moneyflow + 指数K线
"""
import sys, tushare as ts, pymysql
...
MYSQL_PASS = open('/etc/mysql/debian.cnf').read().split("password = ")[1].split('\n')[0]
MYSQL_PASS = MYSQL_PASS.split('#')[0].strip()
...
conn = pymysql.connect(host='127.0.0.1', port=3306, user='debian-sys-maint',
    password=MYSQL_PASS, database='stock_db_v2', charset='utf8mb4',
