...
# ════════════════════════════════════════════
# 方案A：单票上限 = 总资金×15%
# 修改 buy_amount = INIT_CAPITAL * max_pos_pct / 100
# 即每只票最多花总资金的15%，而非剩余现金的15%
# ════════════════════════════════════════════
...
def backtest(start_date='2024-09-02', end_date='2026-06-29'):
    ...
    out = '/tmp/backtest_v13_A_result.json'
    with open(out, 'w') as f:
        json.dump(res, f, ensure_ascii=False, indent=2)
    print(f"\n结果已保存: {out}", flush=True)
    # 方案A：输出到独立文本文件
    with open('/tmp/backtest_v13_A_result.txt', 'w') as rf:
        rf.write(f"V13 方案A 回测结果\n")
        rf.write(f"{'='*50}\n")
        ...
        rf.write(f"{'='*50}\n")
    print(f"结果文本已保存: /tmp/backtest_v13_A_result.txt", flush=True)
    return res
