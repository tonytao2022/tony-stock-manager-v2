#!/usr/bin/env python3
"""
V13 全量回测 — V13四季动态参数矩阵
参数固化自 v2-scores.html V13参数表 (2026-06-29 定型)
...
# ── V13 四季动态参数矩阵（2026-06-29 定型版） ──
V13_PARAMS = {
    'summer':         {'buy':65, 'hold':60, 't1':12, 't2':9,  'p4_min':55, 'p4_ext':15, 'trailing':18, 't2_on':True,  'max_pos_pct':50, 'cool_days':15},
    'spring':         {'buy':65, 'hold':20, 't1':8,  't2':6,  'p4_min':60, 'p4_ext':5,  'trailing':12, 't2_on':True,  'max_pos_pct':15, 'cool_days':15},
    'weak_spring':    {'buy':72, 'hold':20, 't1':8,  't2':6,  'p4_min':65, 'p4_ext':5,  'trailing':12, 't2_on':True,  'max_pos_pct':15, 'cool_days':15},
    'chaos':          {'buy':72, 'hold':25, 't1':10, 't2':8,  'p4_min':65, 'p4_ext':5,  'trailing':12, 't2_on':True,  'max_pos_pct':15, 'cool_days':15},
    'weak_autumn':    {'buy':80, 'hold':15, 't1':5,  't2':4,  'p4_min':999,'p4_ext':5,  'trailing':12, 't2_on':False, 'max_pos_pct':5,  'cool_days':15},
    'autumn':         {'buy':80, 'hold':15, 't1':5,  't2':4,  'p4_min':999,'p4_ext':0,  'trailing':12, 't2_on':False, 'max_pos_pct':8,  'cool_days':15},
    'winter':         {'buy':80, 'hold':10, 't1':5,  't2':4,  'p4_min':999,'p4_ext':0,  'trailing':8,  't2_on':False, 'max_pos_pct':5,  'cool_days':15},
    'chaos_spring':   {'buy':72, 'hold':20, 't1':8,  't2':6,  'p4_min':65, 'p4_ext':5,  'trailing':12, 't2_on':True,  'max_pos_pct':15, 'cool_days':15},
    'chaos_autumn':   {'buy':80, 'hold':15, 't1':5,  't2':4,  'p4_min':999,'p4_ext':0,  'trailing':12, 't2_on':False, 'max_pos_pct':5,  'cool_days':15},
}

SEASON_ALIAS = {
    'summer': 'summer', 'spring': 'spring', 'weak_spring': 'weak_spring',
    'chaos': 'chaos', 'chaos_spring': 'chaos_spring', 'chaos_autumn': 'chaos_autumn',
    'weak_autumn': 'weak_autumn', 'autumn': 'autumn', 'winter': 'winter',
}
...
def simulate_buy_sell(ts_code, buy_date, params, season):
    ...
    price_series = get_hold_dates(ts_code, buy_date, end_date_dt.strftime('%Y-%m-%d'))
    if not price_series:
        return None

    high_since_buy = buy_price
    hold_days = 0
    sell_price, sell_date, exit_reason = buy_price, buy_date, '到期平仓'

    for td, cp in price_series:
        if td <= buy_date:
            continue
        hold_days += 1
        ...
        if t1 > 0 and profit_pct <= -t1:
            ...
            break
        if t2_on and t2 > 0 and profit_pct > 3 and drawdown >= t2:
            sell_price, sell_date, exit_reason = cp, td, 'T2回撤'
            break
        if trailing_stop > 0 and profit_pct > 5 and cp <= high_since_buy * (1 - trailing_stop / 100):
    ...
    return {
        'ts_code': ts_code, 'buy_date': buy_date, 'sell_date': sell_date,
        'hold_days': hold_days, 'buy_price': round(buy_price, 3),
        'sell_price': round(sell_price, 3), 'profit_pct': round(total_profit, 2),
        'exit_reason': exit_reason, 'season': season,
    }
...
def backtest():
    ...
    print(f"📊 回测池: {len(codes)}只", flush=True)

    # 2. 交易日
    cur.execute("""
        SELECT DISTINCT trade_date FROM backtest_score_daily
        WHERE trade_date >= '2024-09-02' AND trade_date <= '2026-06-29'
        ORDER BY trade_date
    """)
    ...
    for day_idx, td_str in enumerate(trade_days):
        ...
        for r in day_scores:
            ...
            if sc >= buy_line:
                candidates.append({'ts_code': r['ts_code'], 'score': sc})
        ...
        for cand in candidates[:max_new]:
            ...
            buy_amount = min(capital * max_pos_pct / 100, capital * 0.5)
            ...
            positions[code] = {
                'buy_date': td_str, 'buy_price': buy_close,
                'shares': buy_shares, 'invested': actual_cost,
                'season': market_season, 'params': params,
            }
            capital -= actual_cost
    ...
    for code, pos in list(positions.items()):
        ...
        if cp and pos['buy_price'] > 0:
            ...
            trades.append({
                ...
                'hold_days': (datetime.strptime(trade_days[-1], '%Y-%m-%d') - datetime.strptime(pos['buy_date'], '%Y-%m-%d')).days,
    ...
    print("=" * 60, flush=True)
    print(f"📊 V13 全量回测结果 ({len(codes)}只, {len(trade_days)}天)", flush=True)
    ...
    out = '/tmp/backtest_v13_result.json'
...
if __name__ == '__main__':
    print("V13 全量回测启动 (620只, 2024-09~2026-06)...", flush=True)
    t0 = time.time()
    backtest()
