...
# ── V13 四季参数矩阵（方案C最终版） ──
# 来源: v2-scores.html 页面V13参数表, 方案A+方案B融合
V13_PARAMS = {
    'summer':         {'buy':65, 'hold':60, 't1':12, 't2':9,  'trailing':18, 'p4_min':55, 'p4_ext':15, 't2_on':True,  'max_pos_pct':20, 'max_total_pct':50, 'cool_days':15},
    'spring':         {'buy':65, 'hold':20, 't1':8,  't2':6,  'trailing':12, 'p4_min':60, 'p4_ext':5,  't2_on':True,  'max_pos_pct':15, 'max_total_pct':35, 'cool_days':15},
    'weak_spring':    {'buy':72, 'hold':20, 't1':8,  't2':6,  'trailing':12, 'p4_min':65, 'p4_ext':5,  't2_on':True,  'max_pos_pct':15, 'max_total_pct':35, 'cool_days':15},
    'chaos':          {'buy':72, 'hold':25, 't1':10, 't2':8,  'trailing':12, 'p4_min':65, 'p4_ext':5,  't2_on':True,  'max_pos_pct':15, 'max_total_pct':20, 'cool_days':15},
    'weak_autumn':    {'buy':80, 'hold':15, 't1':5,  't2':4,  'trailing':12, 'p4_min':999,'p4_ext':5,  't2_on':False, 'max_pos_pct':5,  'max_total_pct':15, 'cool_days':15},
    'autumn':         {'buy':80, 'hold':15, 't1':5,  't2':4,  'trailing':12, 'p4_min':999,'p4_ext':0,  't2_on':False, 'max_pos_pct':8,  'max_total_pct':15, 'cool_days':15},
    'winter':         {'buy':80, 'hold':10, 't1':5,  't2':4,  'trailing':8,  'p4_min':999,'p4_ext':0,  't2_on':False, 'max_pos_pct':5,  'max_total_pct':10, 'cool_days':15},
    'chaos_spring':   {'buy':72, 'hold':20, 't1':8,  't2':6,  'trailing':12, 'p4_min':65, 'p4_ext':5,  't2_on':True,  'max_pos_pct':15, 'max_total_pct':35, 'cool_days':15},
    'chaos_autumn':   {'buy':80, 'hold':15, 't1':5,  't2':4,  'trailing':12, 'p4_min':999,'p4_ext':0,  't2_on':False, 'max_pos_pct':5,  'max_total_pct':15, 'cool_days':15},
}
...
def backtest(start_date='2024-09-02', end_date='2026-06-29'):
    ...
    # 方案C：回撤保护
    peak_capital = INIT_CAPITAL  # 追踪资金峰值
    drawdown_triggered = False   # 回撤保护是否已触发
    max_total_pct_original = {}  # 记录各季节原始总仓位上限
    for k, v in V13_PARAMS.items():
        if 'max_total_pct' in v:
            max_total_pct_original[k] = v['max_total_pct']
    ...
    for day_idx, td_str in enumerate(trade_days):
        ...
        for cand in candidates[:max_new]:
            ...
            # 方案C：单票上限=总资金×max_pos_pct(不是现金), 同时受总仓位上限约束, 含回撤保护
            max_pos_pct = params['max_pos_pct']
            max_total_pct = params.get('max_total_pct', 100)
            
            # 计算已用仓位百分比
            used_capital = INIT_CAPITAL - capital
            used_pct = used_capital / INIT_CAPITAL * 100
            
            # 总仓位上限检查
            if used_pct >= max_total_pct:
                continue
            
            # 方案C核心：用INIT_CAPITAL算单票上限
            buy_amount = INIT_CAPITAL * max_pos_pct / 100
            # 如果下单后超出总仓上限，按剩余空间缩减
            if used_pct + max_pos_pct > max_total_pct:
                remaining = max_total_pct - used_pct
                if remaining < 5:
                    continue
                buy_amount = min(buy_amount, INIT_CAPITAL * remaining / 100)
        ...
        if (day_idx + 1) % 50 == 0 or day_idx == 0:
            ...
            drawdown = (peak_capital - total_val) / peak_capital * 100
            if drawdown > 15 and not drawdown_triggered:
                for k, p in V13_PARAMS.items():
                    if 'max_total_pct' in p and p['max_total_pct'] > 10:
                        p['max_total_pct'] = max(10, int(p['max_total_pct'] * 0.75))
                drawdown_triggered = True
                print(f"  🛡️ 回撤保护触发: 峰值{peak_capital:.0f}→{total_val:.0f}({drawdown:.1f}%), 总仓上限降约25%", flush=True)
    ...
    out = '/tmp/backtest_v13_planC_result.json'
